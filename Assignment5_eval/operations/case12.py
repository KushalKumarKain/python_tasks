print("You have chosen 12. HTMD-3 : High Tension Maximum Demand Temporary Supply \n");
toc = int(input('''
                Choose from
                1 = Energy
                OR
...
if toc==1:
	...
	print("There is only Flat Phase available in this case adn its rate is 705 Paisa/unit so your bill comes out to be Rs", result)
                
if toc==2:
	overallUnits = int(input("Enter the amount of kWs used: "))
	a= int(input("Is it 1. Billing Demand up to contract demand OR 2. Billing Demand in excess of contract demand"))
 
	if a==1:
		result = (overallUnits*25)
		print("For this option, the rate is set to be 25 Rs/KW, so your bill comes to be Rs",result)
	
	elif a==2:
		result = overallUnits*30
		print("For this option, the rate is set to be 30 Rs/KW, so your bill comes to be Rs",result)
